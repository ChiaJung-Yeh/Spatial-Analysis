...
near_mrt_result=data.frame(YouBike=taipei_youbike$StatnNm,
                           MRT=paste(taipei_mrt_station[near_mrt,]$StationID, taipei_mrt_station[near_mrt,]$Zh_tw))
# near_mrt_result
```
...
mrt1=filter(taipei_mrt_station, StationID %in% c("R10", "BR10"))

# choose other 5 MRT stations
mrt2=filter(taipei_mrt_station, StationID %in% c("BL10","R13","G08","BR07","BR13"))
...
near_mrt_k3=data.frame(MRT_nn1=paste(taipei_mrt_station[near_mrt_k3$X1,]$StationID, taipei_mrt_station[near_mrt_k3$X1,]$Zh_tw),
                       MRT_nn2=paste(taipei_mrt_station[near_mrt_k3$X2,]$StationID, taipei_mrt_station[near_mrt_k3$X2,]$Zh_tw),
                       MRT_nn3=paste(taipei_mrt_station[near_mrt_k3$X3,]$StationID, taipei_mrt_station[near_mrt_k3$X3,]$Zh_tw))
near_mrt_k3=cbind(YouBike=taipei_youbike$StatnNm, near_mrt_k3)
datatable(near_mrt_k3, options=list(pageLength=10))
```

The table shown above may have a little mistake, since we put all the transfer stations in the data, namely, the first three nearest MRT station might have two stations that refer to the same station but on the different MRT lines. We can revise them by removing the data with duplicated station name. The work can be done by function `distinct()` from package `dplyr`, if any interested, do the nearest neighbor analysis again to get a more reasonable answer.  
