...
Remember the spatial join operation? Actually, we can use function [`st_join()`](#st_join) to do the same thing. The code is shown below.  
...
In addition, the transformation is implemented on the [sfg](#sfg) and [sfc](#sfc) object, instead of [sf](#sf) (the whole spatial data). Thus, please conduct the transformation on the geometry column in spatial data, such as the form `DATA$geometry`. `DATA` means the spatial data (sf), and use `$` to retrieve the `geometry` (sfc) in the data.  
