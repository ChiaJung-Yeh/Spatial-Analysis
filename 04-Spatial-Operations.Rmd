...
## Nearest Analysis
In practical issue, we often want to know the nearest point of the feature, for instance, the nearest MRT station of the school. One method is to construct the complete distance matrix, and filter the nearest one by using function [`st_distance()`](#geometric-measurement). But obviously, it sounds very time-consuming. Package `sf` provided a function called `st_nearest_feature()` to get the index of the nearest geometry. Let's take Youbike and MRT Station in Taipei for example, and supposed we want to know the nearest MRT station of each YouBike station. <font color="red">*_Data needed has been downloaded previously. You placed the file in the same directory as the R script file._* Click [**here**](https://github.com/ChiaJung-Yeh/Spatial-Analysis/raw/master/data.zip) to re-download the file if it is lost.</font>  
...
```{r nearest_feature_line_map, echo=F, eval=T, warning=F, message=F, fig.width=3, fig.height=3, dpi=170, fig.align="center"}
tmap_mode("view")
...
The first three nearest points are shown in the matrix above. It tells us that the first three nearest MRT station index of the first YouBike station (YouBike1.0_捷運市政府站(3號出口)) are 104 (市政府), 105 (永春), 25 (象山) respectively. The following table lists all the result according to the real station name.  

```{r st_nn_k3_table, echo=F, eval=T, warning=F}
near_mrt_k3=data.frame(near_mrt_k3)
near_mrt_k3=data.frame(MRT_nn1=taipei_mrt_station[near_mrt_k3$X1,]$StatnNm,
                       MRT_nn2=taipei_mrt_station[near_mrt_k3$X2,]$StatnNm,
                       MRT_nn3=taipei_mrt_station[near_mrt_k3$X3,]$StatnNm)
...
```{r convex_hull_map, echo=F, eval=T, warning=F, fig.width=6, fig.height=4, dpi=150, fig.align="center"}
...
ggplot()+
  ...
  geom_sf(data=taipei_youbike, color="#FF9224", size=0.5)+
  theme(panel.background=element_blank(),
        axis.title=element_blank(),
...
```{r convex_hull_youbike_map, echo=F, eval=T, warning=F, fig.width=3, fig.height=3, dpi=200}
tmap_mode("view")
...
From the map above, we can realize the YouBike service area of each MRT station. But we can find that some of the YouBike station are not in any convex hull. What happened? In fact, it is because only two or less YouBike stations are belong to their nearest MRT station, and thus, can not derive the polygon as results.  

## Voronoi Polygon
Voronoi polygon is a partition of a plane into regions close to each of a given set of objects, that is, the distance between any points of the region to its object is the minimum. As a result, Voronoi polygon is also known as nearest neighbor polygons. Here briefly introduces the algorithm to find the Voronoi polygon. First of all, we need to find the [Delaunay triangulation](https://www.wikiwand.com/en/Delaunay_triangulation) in the points set, ensuring to maximize the minimum angle of all the angles of the triangles. Then, get all the perpendicular of the edges to find circumcircle of all triangulation. Finally, keep the perpendicular of the edges that can connect the circumcircles, and the remained line is then Voronoi polygon. The basic process of the algorithm constructing Voronoi polygon is illustrated as the figure below.  

```{r voronoi_figure, echo=F, out.width="100%", fig.align="center"}
include_graphics("./graph/voronoi_figure.png")
```

Function `st_voronoi()` can create Voronoi polygons for a set of points, simply by placing the set of points data in the function. But remember to union all the points in advance by using function [`st_union()`](#unions). Let's dig into the practice by taking Taipei MRT stations in the city center for instance. Here we define the city center as the district of 中山區, 中正區, 大安區, 信義區, 松山區, 大同區, 萬華區, and select the MRT station in these districts. Then, union all the stations we select, and use function `st_voronoi()` to produce the result.  

```{r voronoi_mrt, echo=T, eval=T, warning=F}
...
head(mrt_station_vor)

# extract the geometry whose type is "GEOMETRYCOLLECTION"
...
```{r voronoi_mrt_map, echo=F, eval=T, warning=F, fig.width=4, fig.height=3, dpi=100, fig.align="center"}
windowsFonts(A=windowsFont("標楷體"))
windowsFonts(B=windowsFont("Times New Roman"))
getPalette=colorRampPalette(brewer.pal(7, "Set2"))
ggplot()+
  geom_sf(data=town, aes(fill=TOWNNAME), color=NA)+
  scale_fill_manual(values=getPalette(7))+
  geom_sf(data=st_intersection(mrt_station_vor, st_union(town)), fill=NA)+
  geom_sf(data=st_intersection(taipei_mrt_station, town))+
  theme(panel.background=element_blank(),
        axis.title=element_blank(),
        axis.text=element_blank(),
        axis.ticks=element_blank(),
