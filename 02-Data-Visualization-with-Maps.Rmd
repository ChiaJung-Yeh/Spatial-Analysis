...
ggplot(taipei_village_map)+
  ...
  geom_sf(data=taipei_cycle_path, size=0.1, color="#C4E1E1", alpha=0.8)+
  geom_sf(data=taipei_youbike, aes(size=BIKESCAPAC), color="#6C6C6C")+
...
trans_sta=filter(taipei_mrt_station, duplicated(taipei_mrt_station$Zh_tw) | Zh_tw %in% c("七張","北投"))
taipei_mrt_station=filter(taipei_mrt_station, !(Zh_tw %in% trans_sta$Zh_tw) , !(Zh_tw %in% c("七張","北投")))
...
ggplot(taipei_village_map)+
  ...
  geom_sf_text_repel(data=trans_sta, aes(label=Zh_tw), size=3, nudge_x=5, family="A", box.padding=0.3)+
